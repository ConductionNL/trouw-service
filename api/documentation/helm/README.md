...
```bash
helm install my-trouw-service ConductionNL/trouw-service --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `trouw-service` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `trouw-service` |
